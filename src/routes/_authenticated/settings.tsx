...
function SettingsPage() {
  ...
  const save = async () => {
    if (!me) return;
    const { error } = await supabase.from("profiles").update({ display_name: displayName, bio }).eq("id", me.id);
    ...
    <div className="max-w-xl mx-auto px-3 md:px-6 py-4 space-y-6">
      ...
      <section className="card-flat p-4 space-y-3">
        ...
        {me?.subscription_tier === "plus" ? (
          <>
            <p className="text-sm text-muted-foreground">You're on Plus. 48h stories, 6 pinned posts, hide-from-feed posts, super hearts.</p>
            <Button variant="outline" onClick={downgrade}>Cancel Plus</Button>
          </>
        ) : (
          <>
            <p className="text-sm text-muted-foreground">Extend stories to 48h, pin up to 6 posts, super hearts, hide-from-feed posts, and more.</p>
