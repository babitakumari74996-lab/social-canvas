...
function StoryViewer() {
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      const { data: p } = await supabase.from("profiles").select("id, username, avatar_url, close_friends").eq("username", username).maybeSingle();
      if (!p) return null;
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      await supabase.from("story_views").upsert({ story_id: current.id, viewer_id: userId }, { onConflict: "story_id,viewer_id" });
  ...
  if (!data) return <div className="fixed inset-0 bg-black text-white flex items-center justify-center">Loading…</div>;
  if (stories.length === 0) {
    return (
      <div className="fixed inset-0 bg-black text-white flex items-center justify-center">
        <div className="text-center">
          <p>No active stories</p>
          <Link to="/feed" className="text-primary text-sm">Back</Link>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="fixed inset-0 bg-black text-white flex flex-col z-50">
      ...
      <div className="flex items-center justify-between px-3 py-2">
        ...
        <button onClick={() => navigate({ to: "/feed" })}><X className="h-5 w-5" /></button>
      </div>
      <div className="flex-1 flex items-center justify-center relative">
        {media.data && <img src={media.data} alt="" className="max-h-full max-w-full object-contain" />}
        <button className="absolute inset-y-0 left-0 w-1/3" onClick={() => setIdx(Math.max(0, idx - 1))}>
          <ChevronLeft className="h-6 w-6 opacity-0 hover:opacity-70 mx-2" />
        </button>
        <button className="absolute inset-y-0 right-0 w-1/3" onClick={() => idx < stories.length - 1 ? setIdx(idx + 1) : navigate({ to: "/feed" })}>
