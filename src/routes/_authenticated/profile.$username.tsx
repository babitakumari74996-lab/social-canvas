...
function GridImg({ path, pinned }: { path: string; pinned?: boolean }) {
  ...
  return (
    <div className="aspect-square bg-muted overflow-hidden relative">
      ...
      {pinned && (
        <Pin
          className="absolute top-2 right-2 h-4 w-4 text-white drop-shadow"
          fill="currentColor"
        />
      )}
    </div>
...
function ProfilePage() {
  ...
  const toggleMute = async () => {
    ...
    await supabase
      .from("profiles")
      .update({ muted_users: Array.from(muted) })
      .eq("id", me.id);
  ...
  const toggleCloseFriend = async () => {
    ...
    await supabase
      .from("profiles")
      .update({ close_friends: Array.from(cf) })
      .eq("id", me.id);
    qc.invalidateQueries({ queryKey: ["profile"] });
      ...
      <header className="flex items-start gap-4 md:gap-8 mb-6">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex gap-4 mt-2 text-sm">
            <span>
              <b>{counts?.posts ?? 0}</b> posts
            </span>
            <span>
              <b>{counts?.followers ?? 0}</b> followers
            </span>
            <span>
              <b>{counts?.following ?? 0}</b> following
            </span>
          </div>
          {profile.display_name && (
            <div className="mt-2 font-medium text-sm">{profile.display_name}</div>
          )}
          {profile.bio && (
            <div className="text-sm text-muted-foreground whitespace-pre-line">{profile.bio}</div>
          )}
            ...
            {isMe ? (
              ...
              <>
                ...
                <Button
                  size="sm"
                  variant="outline"
                  onClick={() => navigate({ to: "/dm/$userId", params: { userId: profile.id } })}
                >
                  Message
                </Button>
                ...
                <Button size="sm" variant="ghost" onClick={toggleCloseFriend}>
                  {me?.close_friends?.includes(profile.id)
                    ? "★ Close friend"
