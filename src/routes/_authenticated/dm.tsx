...
function DMLayout() {
  ...
  return (
    <div className="max-w-4xl mx-auto flex h-[calc(100vh-8rem)] md:h-[calc(100vh-3rem)]">
      <aside
        className={`${inThread ? "hidden md:flex" : "flex"} flex-col w-full md:w-80 border-r border-border`}
      >
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs text-muted-foreground truncate">
                  {t.last.content ?? "media"} ·{" "}
                  {formatDistanceToNowStrict(new Date(t.last.created_at))}
                </div>
              </div>
            </Link>
          ))}
